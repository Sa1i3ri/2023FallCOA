...
public class FPUAddTest {
    ...
    public void fpuAddTest2() {
        float num1 = 1.1F;
        float num2 = 2.5F;
        src = new DataType(Transformer.floatToBinary(String.valueOf(num1)));
        dest = new DataType(Transformer.floatToBinary(String.valueOf(num2)));
        result = fpu.add(src, dest);
        assertEquals(Transformer.floatToBinary(String.valueOf(num1 + num2)), result.toString());
    }
    ...
    public void fpuAddTest4() {
        ...
        float num2 = 2.5F;
        src = new DataType("00000000000000000000000000000001");
        dest = new DataType("00000000100000000000000000000000");
        result = fpu.add(src, dest);
        assertEquals("00000000100000000000000000000001", result.toString());
    }
    ...
    public void fpuAddTest9() {
        ...
        for (float opr1 : input) {
            for (float opr2 : input) {
                ...
                System.out.println(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(opr1))));
                System.out.println(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(opr2))));
                System.out.println();
                result = fpu.add(src, dest);
