...
public class FPUMulTest {
	...
	public void fpuMulTest2(){
		...
		dest = new DataType("01111111100000000000000000000000");
		result = fpu.mul(src, dest);
		assertEquals( "01111111110000000000000000000000", result.toString() );
	}
	@Test
	...
	public void fpuMulTest5(){
		...
		result = fpu.mul(src, dest);
		assertEquals( "00110100100000000000000000000001", result.toString() );
	}
	...
	public void fpuMulTest9() {
		...
		for (int i = 0; i < input.length; i++) {
			for (int j = 0; j < input.length; j++) {
				...
				System.out.println(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(input[i]))));
				System.out.println(Transformer.intToBinary(Integer.toString(Float.floatToIntBits(input[j]))));
				System.out.println();
				result = fpu.mul(src, dest);
