...
public class ALU {
    ...
    public String mul(String src, String dest,int length) {
        //int length = 32;
        //srcStr：33位，乘数
        length = length+1;
        StringBuilder srcStr = new StringBuilder("0"+src.toString()+'0');
        //被乘数
        StringBuilder destStr = new StringBuilder("0"+dest.toString());
        StringBuilder result = new StringBuilder();
        for(int i =0;i<length;i++){
            result.append('0');
        }
        ...
        for(int i = srcStr.length()-2;i>=0;i--){
            ...
            if(reference.equals("01")){
                //相加
                result = adder(result.substring(0,length),destStr.toString(),0,length).append(result.substring(length));
            }else if(reference.equals("10")){
                //相减
                result = adder(result.substring(0,length),negation(destStr).toString(),1,length).append(result.substring(length));
            }
            //右移
            result.insert(0,result.charAt(0));
        }
        ...
        return result.substring(result.length()-2*(length-1));
    }
    ...
    public String div(String src,String dest){
        //无余数除法
        StringBuilder srcStr= new StringBuilder(src);
        StringBuilder destStr= new StringBuilder(dest);
        ...
        for(int i=0;i<srcStr.length();i++){
            StringBuilder temp = suber(srcStr.toString(),destStr.substring(0,srcStr.length()),srcStr.length());
            if(temp.charAt(0)=='1'){
                //不够减
                result.append('0');
            }else{
                result.append('1');
                destStr = temp.append(destStr.substring(srcStr.length()));
            }
            destStr.deleteCharAt(0);
            //result.append('0');
        }
