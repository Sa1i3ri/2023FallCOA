...
public class MMU {
    ...
    private String toRealLinearAddr(String logicAddr) {
        StringBuilder result = new StringBuilder(logicAddr.substring(0,16)).append("0000");
        while (result.length()<32){
            result.insert(0,'0');
        }
        StringBuilder offset = new StringBuilder(logicAddr.substring(logicAddr.length()-16));
        while (offset.length()<32){
            offset.insert(0,'0');
        }
        result = new StringBuilder(alu.add(new DataType(result.toString()),new DataType(offset.toString())).toString());
    ...
    private String toSegLinearAddr(String logicAddr) {
        int segIndex = Integer.parseInt(Transformer.binaryToInt("0"+logicAddr.substring(0,13)));
        //base：32位
        String base = String.valueOf(memory.getBaseOfSegDes(segIndex));
        //offset：32位
        String offset = logicAddr.substring(logicAddr.length()-32);
     ...
     * 段页式下的线性地址转物理地址
     *
     * @param linearAddr 32位，前20位是页表号，后16位是偏移量
     * @return 32-bits 物理地址
     */
    private String toPagePhysicalAddr(String linearAddr) {
        int vpageNO = Integer.parseInt(Transformer.binaryToInt("0"+linearAddr.substring(0,20)));
        StringBuilder offset = new StringBuilder(linearAddr.substring(20));
        StringBuilder base = new StringBuilder(String.valueOf(memory.getFrameOfPage(vpageNO)));
        ...
        String result = alu.add(new DataType(base.toString()) , new DataType(offset.toString())).toString();
