...
public class Memory {
    ...
    private final PageItem[] pageTbl = new PageItem[Disk.DISK_SIZE_B / Memory.PAGE_SIZE_B]; // 总页表，磁盘和内存

    private boolean[] pageValid = new boolean[MEM_SIZE_B / PAGE_SIZE_B];//内存
    ...
    public void seg_load(int segIndex) {
        //写GDT
        SegDescriptor cur = getSegDescriptor(segIndex);
        Arrays.fill(cur.base,'0');
        Arrays.fill(cur.limit,'1');
        cur.validBit = true;
        //是否分页决定粒度多少，分页的话，粒度为一页
        cur.granularity = PAGE;
        if(PAGE){
            return;
        }

        //写入
        int len =Integer.parseInt(Transformer.binaryToInt("0"+String.valueOf(cur.limit))) ;
        //cur.base = "0000..."
        write(String.valueOf(cur.base),len,disk.read(String.valueOf(cur.base),len));
    ...
    public void page_load(int vPageNo) {
        //虚拟页号是磁盘里的号码
        String diskAddr = Transformer.intToBinary(String.valueOf(vPageNo * PAGE_SIZE_B));
        int pageNO = 0;
        ...
        String memoryAddr = Transformer.intToBinary(String.valueOf(pageNO * PAGE_SIZE_B));
        write(memoryAddr,PAGE_SIZE_B,disk.read(diskAddr,PAGE_SIZE_B));

        getPageItem(vPageNo).pageFrame = memoryAddr.substring(0,20).toCharArray();
